...
private List<Sql> connent(int num) {
  def result = []
  1.upto(num) {result.add(Sql.newInstance("jdbc:mysql://" + PropUtil.host + ":3309/benchmark_insert", "test", "test", "com.mysql.jdbc.Driver"))}
  return result
}

def clients = connent(client_num)

def s = new StressTest(count: 10000000)
s.add({
